...
to evaluate. CURRENT-TIME is the optional timestamp to use as now."
  ...
  (let* ((elapsed-days (max 0 (seconds-days (timestamp-difference current-time (card-last-review card)))))
         (decay (- (aref (scheduler-parameters scheduler) 20)))
         (factor (- (expt 0.9 (/ 1 decay)) 1))
         (retrievability (expt (1+ (/ (* factor elapsed-days) (card-stability card))) decay)))
    retrievability))
...
strength."
  (let* ((decay (- (aref (scheduler-parameters scheduler) 20)))
         ...
         (interval (* (/ stability factor) (1- (expt (scheduler-desired-retention scheduler) (/ decay))))))
